...
void Servo::zServoOn() {
  if (this->attach(0) >= 0) {
    int angletoset;
    //
    SERIAL_ECHOLN(" angleset value : ");
    angletoset = constrain(floor(this->startAngle + ((this->pressure-this->minPressure)/this->pressurePerAngle)),0,180);
    SERIAL_ECHOLN(angletoset);
    if(this->invert_z_servo)
    {
      angletoset = map(angletoset,0,180,180,0);
      SERIAL_ECHOLN(angletoset);
    }
    SERIAL_ECHOLN(angletoset);
    this->write(angletoset);
    delay(150);
    #if ENABLED(DEACTIVATE_SERVOS_AFTER_MOVE)
      delay(SERVO_DEACTIVATION_DELAY);
      this->detach();
...
void Servo::zServoOff() {
  if (this->attach(0) >= 0) {
    int angletoset;
    SERIAL_ECHOLN(" angleset value : ");
    angletoset = constrain(this->lift_angle,0,180);
    SERIAL_ECHOLN(angletoset);
    if(this->invert_z_servo)
    {
      angletoset = map(constrain(this->lift_angle,0,180),0,180,180,0);
      SERIAL_ECHOLN(angletoset);
    }
    SERIAL_ECHOLN(angletoset);
    this->write(angletoset);
