...
inline void gcode_G4() {
...
#if HAS_Z_SERVO
  /**
   * M370: set servo parameter
   * M minimun pressure
   * P: maximum pressure
   * A: pressure/deg
   * S: start angle
   * D: set direct angle for adjustement prupose  0 would be top and 180 down
   * U: set pressure usage
   * L: lift angle
   * I : invert
   */
  inline void gcode_M370() {
    if(code_seen('M'))
    {
      Zservo.minPressure = code_value_short();
    }
    if(code_seen('P'))
    {
      Zservo.maxPressure = code_value_short();
    }
    if(code_seen('A'))
    {
      Zservo.pressurePerAngle = code_value_short();
    }
    if(code_seen('S'))
    {
      Zservo.startAngle = code_value_short();
    }
    if(code_seen('U'))
    {
      unsigned int pressure;
      pressure = constrain(code_value_long(),0,300);
      Zservo.pressure = pressure;
    }
    if(code_seen('L'))
    {
      Zservo.lift_angle = code_value_short();
    }
    if(code_seen('I'))
    {
      Zservo.invert_z_servo = code_value_short();
    }
    if(code_seen('D'))
    {
      int angle;
      angle = constrain(code_value_short(),0,180);
      if(Zservo.invert_z_servo)
      {
        Zservo.move(map(angle,0,180,180,0));
      }
      else
      {
        Zservo.move(angle);
    ...
    if(code_seen('F'))
    {
      Zservo.move(code_value_short());
    }
    SERIAL_PROTOCOLPGM(" MinPressure:");
    SERIAL_PROTOCOL(Zservo.minPressure);
    SERIAL_PROTOCOLPGM(" MaxPressure:");
    SERIAL_PROTOCOL(Zservo.maxPressure);
    SERIAL_PROTOCOLPGM(" Pressure per angle:");
    SERIAL_PROTOCOL(Zservo.pressurePerAngle);
    SERIAL_PROTOCOLPGM(" Start angle:");
    SERIAL_PROTOCOL(Zservo.startAngle);
    SERIAL_PROTOCOLPGM(" pressure:");
    SERIAL_PROTOCOL(Zservo.pressure);
    SERIAL_PROTOCOLPGM(" Lift angle:");
    SERIAL_PROTOCOL(Zservo.lift_angle);
    SERIAL_PROTOCOLPGM(" invert Z:");
    SERIAL_PROTOCOL(Zservo.invert_z_servo);
    
  }
  /**
   * G371: set servo on position
   */
  inline void gcode_G371() {
    st_synchronize();
    Zservo.zServoOn();
  }
  /**
   * G372: set servo off position
   */
  inline void gcode_G372() {
    st_synchronize();
    Zservo.zServoOff();
  }
#endif
